...
export async function reviewPost(formData: FormData) {
  ...
  const { data: post, error } = await supabase.from("posts").update({ status: approve ? POST_STATUS.APPROVED : POST_STATUS.REJECTED, reviewed_by: userId, reviewed_at: new Date().toISOString(), reject_reason: approve ? null : reason || null }).eq("id", postId).select("author_id, title_en").single();
  if (error || !post) {
    console.error("[admin/reviews] reviewPost update failed", error);
    redirect("/admin/moderation?toast=actionFailed");
  }
  await sendNotificationEmail(supabase, post.author_id, approve ? "post_approved" : "post_rejected", { title: post.title_en, reason });
...
export async function reviewMessage(formData: FormData) {
  ...
  const { data: message, error } = await supabase.from("inquiry_messages").update({ review_status: forward ? MESSAGE_REVIEW_STATUS.FORWARDED : MESSAGE_REVIEW_STATUS.REJECTED, reject_reason: forward ? null : reason || null, admin_feedback: feedback || null, reviewed_by: userId, reviewed_at: new Date().toISOString() }).eq("id", messageId).select("sender_id, inquiry_id").single();
  if (error || !message) {
    console.error("[admin/reviews] reviewMessage update failed", error);
    redirect("/admin/inquiries?toast=actionFailed");
  }
...
export async function reviewBadgeApplication(formData: FormData) {
  ...
  const { data: application, error } = await supabase.from("badge_applications").update({ status: approve ? "approved" : "rejected", reject_reason: approve ? null : reason || null, reviewed_by: userId, reviewed_at: new Date().toISOString() }).eq("id", applicationId).select("profile_id").single();
  if (error || !application) {
    console.error("[admin/reviews] reviewBadgeApplication update failed", error);
    redirect("/admin/badges?toast=actionFailed");
  }
  await sendNotificationEmail(supabase, application.profile_id, approve ? "badge_approved" : "badge_rejected", { reason });
